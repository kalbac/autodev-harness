...
> ## ✅ s46 — **overnight PRESENCE TOGGLE (ADR-004 unattended half, 2nd slice)** — daemon wiring + global settings store + sidebar UI — built subagent-driven + **4-pass codex(luna)-gated** + LIVE-PROVEN (real daemon + Chrome) → branch `autodev/s46-overnight-presence-toggle` (**PR pending — agent to open, operator approves merge in-turn**)
> Attended. Operator picks (4 one-at-a-time): full slice · global master AND per-project opt-in (runs on the AND) · plain persistent boolean · sidebar-footer placement (a documented divergence from ADR-004's literal "top bar").
> - **The s46 discovery:** the s45 overnight supervisor was **UNREACHABLE** from the daemon — `runOrSupervise` had one caller (CLI `run`), while both daemon run paths called `conductor.run` directly. s45's headless live-proves never surfaced it. This slice wires it in.
> - **Store + semantics:** daemon-global operator PRESENCE in `~/.autodev/settings.json` (`overnight.enabled`), never-throws load, atomic tmp+rename save (symlink-guarded on BOTH dest and tmp). Per-project opt-in stays `autonomy.overnight.enabled` in the project's `.autodev/config.yaml`. Overnight runs on `shouldSupervise` = presence × opt-in; **read-through** (presence re-read fresh per trigger, no cache), every ambiguity fails toward ATTENDED. `countOptedIn` → intersection count for the UI.
> - **UI:** `switch` primitive vendored (base-nova); sidebar-footer `OvernightToggle` with a three-state sub-line (`off · attended` / `on · N of M projects` / `on · no project opted in`), collapsed-rail icon+tooltip variant; per-project opt-in row in Project Settings (`AUTONOMY` section); `GET`/`PATCH /settings`; a **new `mergeConfigYaml` autonomy branch** (the Task-7 worker caught that opening the schema alone would make PATCH 200 while the value silently never landed — the exact silent-no-op class this slice removes).
> - **Gate:** 1161 tests/3 skip (+42), typecheck + build(root+ui) green. **codex gpt-5.6-luna 4 passes**: NOT SAFE → (symlink-tmp guard + `trigger` bounded-default) → NOT SAFE (`{once:false,drain:false}` still unbounded; `rm recursive` LOW) → NOT SAFE (`typeof number` admits NaN/Infinity) → **SAFE** (`Number.isFinite`). Every fix re-critic'd; never self-certified. New gotcha `[conductor/once-precedes-drain]` (69→70).
> - **LIVE-PROVEN (real daemon + Chrome, operator-attended):** all three sub-line states with disk verification (`settings.json` / `config.yaml` / API counts), collapsed-rail tooltip, then a **zero-LLM park contrast** through the REAL `run` path — same command, presence OFF → `conductor.run` branch (no journal), presence ON → `superviseOvernight` branch (one `constitution → park` decision-journal line). The toggle genuinely gates the assembled daemon's routing. Test repo restored to baseline.
> - **NEXT (s47):** open the s46 PR + merge (operator in-turn). Then the DOCS + new-goals discussion the operator flagged. Remaining ADR-004 unattended half: morning report, north-star concept doc, mandatory anti-drift — each its own brainstorm→spec→plan. Carried: agent-ci synthetic `GITHUB_REPO`; the s45 PR (`autodev/s45-carried-items`) status to confirm.
>
> ## ✅ s45 — **2 carried fixes + FIRST unattended-autonomy slice (overnight escalation handling)** — built subagent-driven + **4-pass codex(luna)-gated** + LIVE-PROVEN (real daemon, twice) → branch `autodev/s45-carried-items` (16 commits, **PR pending — agent to open, operator approves merge in-turn**)
> Attended. Operator skipped GitHub auto-merge setup (no `--auto`); "small items first, then the unattended brainstorm".
> - **safeLog (`2c00ba7`)** — `server.ts` base `log` fail-closed file-wide + `safeErrorText` + 9 swaps; closes s44 `[ts/fail-closed]` backlog. codex APPROVE.
> - **dotted-id SPA-fallback (`c873c82`)** — extension-agnostic fallback (missing non-`/assets/` → index.html) resolves `[ui/dotted-id-breaks-spa-reload]` for run/task/thread ids; `/assets/*` still 404s; security preserved. codex APPROVE.
> - **shadcn AI-helpers recon (`d6c518b`)** — both are offline test-fixtures gated on a `useChat` runtime we lack; backlogged a **chat-runtime → TanStack AI + AG-UI** migration (its own future brainstorm).
> - **Unattended-overnight-escalation-handling (ADR-004 first slice)** — brainstorm→spec(`e02b68b`)→plan(`ccab416`)→build (Sonnet 5, Impl A+B). Deterministic (no-LLM) `superviseOvernight` above-gate loop: reason-routes `EscalationType` (retryable → auto-rework via reply-B triple bounded by `maxAutoReworks`; else park), journals to `.autodev/decision-journal.ndjson`, inert unless `autonomy.overnight.enabled`. **codex gpt-5.6-luna 4 passes** (BLOCKED→4 Major→fix→D partial→fix→cross-run over-budget→**saga fix**→PASS+nit→fix). New gotcha `[autonomy/budget-saga-order]` (68→69).
> - **Gate**: 1119 tests/3 skip, typecheck+build(root+ui) green. **LIVE-PROVEN twice**: zero-LLM park proof (2 correct park lines, parseReworkCount fail-closed live) + bounded auto-rework proof (disagreement docblock → auto-rework → worker → critic clean → **real commit `d67674e`**). Test repo at baseline.
> - **NEXT (s46)**: open the s45 PR + merge (operator in-turn). Then the rest of ADR-004's unattended half (top-bar overnight toggle, morning report, north-star doc, mandatory anti-drift) — each its own brainstorm→spec→plan. Smaller carried item still open: agent-ci synthetic `GITHUB_REPO` for non-GitHub repos.
>
> ## ✅ s44 — **gpt-5.6-luna PROMOTED as critic (calibrated) + reply-B poison-fix SHIPPED** — both codex-gated + LIVE-PROVEN end-to-end (real DONE commits) → branch `autodev/s44-gpt56-critic-and-poison-fix` (PR pending CI)
> Autonomous overnight session (operator present for (b) only, then full autonomy granted). Two deliverables, both live-proven through the REAL daemon; the browser run proves both at once.
> - **(b) — gpt-5.6-luna is the critic now (calibration).** Replayed the EXACT production critic invocation (real `buildCriticPrompt`, effort `high`, `-o` outfile) on the 4 known cases (method-id bug→broken, its fix→clean, correct getter→clean [ADR-005], load-order→broken), 3 rounds each, for `gpt-5.5` (baseline) + all three 5.6 variants. Baseline reproduced the known verdicts 4/4 → fixtures faithful. **Result: `luna` 12/12 correct** (matches 5.5, sharper on real bugs, cheapest) → **promoted**; **`sol`** deterministically FALSE-BLOCKS the correct fix (3/3, worst gate profile + most expensive); **`terra`** unreliably catches the real bug (~1-in-3 miss). Applied: `schema.ts` critic default `gpt-5.5`→`gpt-5.6-luna` (kills the s43 un-pinned drift onto sol), detect catalog offers the 5.6 variants for the UI picker, test-repo config pinned to luna. Full methodology + table: `docs/wiki/critic-model-calibration-s44.md`; gotcha `[critic/gpt-5.6-variant-behaviors]` (67→68). commit `66f322f`.
> - **(a) — reply-B poison-fix (`[rework/reply-b-poisons-maxrounds-exhausted-task]`, RESOLVED).** The `POST /escalations/:id/reply` choice-B branch now resets the circuit-breaker attempt budget (`repo.setAttempts(id, 0)`) after the escalated→pending move and BEFORE the drain, so a round-exhausted rework re-claims fresh (0→1, under maxAttempts) and reaches the worker instead of the poison-pill quarantining it in ~80ms. Scoped to choice B + a real move; best-effort (never breaks the 200); the in-loop retry poison-pill unchanged. TDD: 2 unit + 1 real repo+scheduler integration test. commit `07797c8`. codex gpt-5.6-luna gate: 1 Medium (best-effort catch could re-throw if `String(err)`/`log` throws — the `[ts/fail-closed]` class) **declined with rationale** (it is the file's convention across 10 sites, default `log` is a no-op, not a regression of this diff) + backlogged a file-wide `safeLog` refactor.
> - **Gate:** 1089 tests / 3 skip (+3), typecheck + build green (root+ui) on both commits.
> - **LIVE-PROVEN end-to-end through the REAL daemon (+ browser):** (1) **deterministic (a) proof** — seeded an escalated task at `attempts=3` (= maxAttempts, the exact poison precondition) → API reply-B → **attempts reset 3→0** (server log) → drain re-claimed → conductor claim `0+1=1` (NO poison) → worker (sonnet) wrote a correct docblock reading the seeded feedback → **luna `clean 0.98`** → agent-ci 5/5 green → **COMMIT `4fae4d2`** → DONE. The contrast is in ONE conductor.log: s43's pre-fix `[ESCALATE] (poison)` at 23:28:23 vs s44's worker-run at 01:05:37; quarantine stayed empty. (2) **browser run** — a thread launched in Chrome (chat → streaming plan → Launch → self-narrating run) reached DONE with **luna `clean 0.99`** → agent-ci 5/5 → **COMMIT `e3073aa`**; ROLES + the launch chip both show `codex · gpt-5.6-luna · high` (so (b) is live in the UI too).
> **NEXT (s45):** push branch → PR → merge on green CI (autonomous). Then the operator picks: **UNATTENDED-HALF AUTONOMY BRAINSTORM** (ADR-004 — the big remaining project half, do NOT start until he says so); or smaller carried items (file-wide `safeLog` in server.ts [ts/fail-closed]; agent-ci synthetic `GITHUB_REPO` for non-GitHub repos; the dotted-id SPA-fallback ROOT fix).
>
> ## ✅ s43 — **s42 reply-B cycle LIVE-PROVEN (operator-attended) + blocked-state SHIPPED** (narrator `blocked` status + reply-B re-arm) — codex-gated + live-proven → **PR #74 MERGED to main (`137a42d`), CI 4/4**
> Two deliverables, both operator-attended live-proven end-to-end through the REAL daemon + Chrome on `woodev-shipping-plugin-test`.
> - **#1 — the s42 reply-B daemon cycle LIVE-PROVEN (closes the one deliberate s42 gap).** Launched a cart-fee task → worker wrote a real **method-id parsing** bug → **critic `broken 0.72`** (codex gpt-5.5) → escalate → **`critic-feedback.md` written** (s42 fix part a) → operator clicked **B** → **`onReplyRework`→drain→re-run at round 0** unconditionally read the feedback (parts b+c) → worker fixed EXACTLY the flagged bug (`explode(':')` + prefix compare + a comment citing WC `method_id:instance_id`) → **critic `clean 0.79` → gate → COMMIT `af8d856` → DONE**. The full escalate→reply-B→feedback→converge story watched live (s42 proved only the data path). Env detour: cleared a memory-pressure pileup (**69 stale processes**, gotcha s34) that hung the opus decompose (5+min vs ~13s); after cleanup, normal.
> - **#2 — blocked-state SHIPPED (resolves `[narrator/escalated-run-not-terminal]`, un-built since s40).** An escalation-parked run (no task `active`/`pending`, ≥1 `escalated`) is not terminal → the narrator kept the thread `running` + idle-ticked every 1.5s forever. Fix (commits `3919377`+`0d78b52`): distinct **`blocked`** thread status + one-shot "waiting on your reply" note + `stop()`; **reply-B re-arm** (`onReplyRework(taskId)` → composition finds the blocked thread by `run_id` → restarts a narrator bound to the same run via `boundRunId`, silent baseline → narrates only post-reply progress); UI `blocked` glyph (PauseCircle/uncertain) + ThreadView badge. **1086 tests (+4), typecheck root+ui, both bundles green.** codex gpt-5.5 gate `uncertain`: 2 race findings verified FALSE against `server.ts` (moveTask-before-onReplyRework ordering; synchronous `startNarrator` stop-then-set), 1 observability closed by an INFO re-arm log. **LIVE-PROVEN**: pickup-fee task escalated after 3 rounds → thread **BLOCKED** (badge + amber glyph + note + narrator stopped) → reply-B → **`running` re-arm** (INFO log fired) → terminal handled (`error`, no wedge).
> - **Live-prove #2 earned its keep → new gotcha `[rework/reply-b-poisons-maxrounds-exhausted-task]` (GOTCHAS 66→67):** reply-B on a **round-exhausted** escalation does NOT re-run the worker — the drain re-claims it and the conductor's poison-pill **immediately re-escalates as `poison`→quarantine** (~80ms, no worker), because the attempt budget isn't reset on reply-B. (s42's cart-fee escalated after ONE round → budget not exhausted → reply-B re-ran fine.) Conductor behaviour, NOT a blocked-state defect; candidate fix (reset budget on operator reply-B) deferred.
> - **Codex model note:** the codex CLI silently defaulted to `gpt-5.6-sol` (new, provisioned) but the installed CLI was too old → `400 requires a newer Codex`; the gate ran on `gpt-5.5`. **Operator upgraded the codex CLI at end of s43 → gpt-5.6 Sol/Terra/Luna now invokable** — evaluate one as the critic per the FUTURE-BACKLOG calibration protocol (PIN the model explicitly; the gate drifts onto the default otherwise).
> **NEXT (s44):** (0) merge the blocked-state PR if not already; then operator picks: (a) fix `[rework/reply-b-poisons-maxrounds-exhausted-task]` (reset attempt budget on reply-B); (b) **evaluate gpt-5.6 (Sol/Terra/Luna) as critic** — calibrate vs 5.5 on the known cases (method-id→broken 0.72, its fix→clean 0.79, correct getter→clean 0.82, load-order→broken 0.76); (c) the **UNATTENDED-HALF AUTONOMY brainstorm** (ADR-004, still deferred — the big remaining half).
>
> ## ✅ s42 — **two s41 structural findings FIXED + gated + verified + MERGED** (critic→correctness gate [ADR-005] + reply-B rework feedback) — autonomous overnight session → **PR #73 merged (`ffefeb7`), CI green 4/4**
> Operator ordered priorities #1 then #2, then handed the session to me autonomously ("доведи до конца... не пиши готово пока сам не проверишь end-to-end"). Branch `autodev/s42-critic-correctness-gate` → **PR #73 MERGED to main (`ffefeb7`), CI 4/4 (ubuntu+windows × node 20/22)**.
> - **#1 — critic is a correctness gate, coverage is mechanical (ADR-005, commit `21c8019`).** Resolved `[gate/critic-before-ci-blocks-testless-repos]`. **Prompt-only** change to `src/critic/prompt.ts`: removed "a missing guard/test" as a clean-blocker; added a "coverage is not a defect, correctness IS your job" section; checklist #2 diagnostic-not-decisive. Coverage stays exclusively the mechanical gate's job (zones + mutation-verified guards) + agent-ci. Broken-contract / fabricated-proof / logic-regression remain hard `broken`. **Rejected reordering CI before the critic** (agent-ci expensive+off-by-default; the ordering problem was a symptom of the unsatisfiable critic). **Proven END-TO-END on the REAL codex gpt-5.5 critic** (2 runs on s41-shaped PHP diffs): correct getter, no test → **clean 0.82** (was `broken 0.73` in s41); real load-order silent-skip bug → **broken 0.76**. codex gate: 1 blocker (false-positive: mis-parsed inline diff) declined + 1 minor (over-absolute heading) applied+re-verified.
> - **#2 — reply-B rework carries the critic's objection (commit `3fd977b`).** Resolved `[rework/reply-b-drops-critic-feedback]`. (a) escalation branch writes `critic-feedback.md`; (b) round-0 read is unconditional (fresh claim → no file → undefined); (c) reply-B fires `onReplyRework`→`conductor.run({drain})` (R1-thin, guarded). **Verified:** 3 conductor + 5 server unit tests + a **REAL repo+scheduler INTEGRATION test** (feedback survives escalated→pending→re-claim, which fakes can't prove) + worker-adapter forwarding already covered + **live daemon boot-smoke** (new dist assembles ProjectView w/ onReplyRework, serves on :4320). codex gate: 1 blocker (same inline-diff false-positive) declined + 1 major (unguarded fire-and-forget hook could break the 200 response) applied (try/catch at server boundary + test).
> - **Gate summary:** 1082 tests / 3 skip, root typecheck + build green on both commits. New gotcha `[critic/codex]` `codex-inline-diff-strips-quotes-false-blocker` (hit TWICE — the inline-diff mangles string quotes → false "invalid syntax" blocker; verify against typecheck/build/tests, decline). GOTCHAS 65→66; two entries marked RESOLVED.
> - **NOT DONE (deliberately, per overnight "stop before expensive unsupervised live runs"):** the full LLM reply-B→rework→clean **daemon** cycle (needs an unattended worker+critic run) — the fix's data path is fully proven without it; flagged for an **operator-attended** live-prove. **NEXT (s43):** operator-attended reply-B daemon live-prove if he wants it; then chat polish (`[narrator/escalated-run-not-terminal]` blocked-state, still un-built) then unattended-half autonomy (ADR-004). Smaller open s41 items still open (agent-ci synthetic `GITHUB_REPO`, dotted-id SPA-fallback root fix).
>
